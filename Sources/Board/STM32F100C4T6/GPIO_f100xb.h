#ifndef _GPIO_F100XB_H_
#define _GPIO_F100XB_H_
...
namespace Hardware
{
    ...
    template <
        GPIO_Ports Port,
        unsigned char PIN>
    void GPIO<Port, PIN>::InitInputFloating()
    {
        if constexpr (Port == GPIOA_BASE)
        {
            RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;
        ...
        if constexpr (PIN <= 7)
        {
            ((GPIO_TypeDef *)Port)->CRL &= ~(0x3UL << (4 * PIN));
            ((GPIO_TypeDef *)Port)->CRL |= 0x1UL << (4 * PIN + 2);
            ((GPIO_TypeDef *)Port)->CRL &= ~(0x2UL << (4 * PIN + 2));
        }
        else
        {
            ((GPIO_TypeDef *)Port)->CRH &= ~(0x3UL << (4 * (PIN - 8)));
            ((GPIO_TypeDef *)Port)->CRH |= 0x1UL << (4 * (PIN - 8) + 2);
            ((GPIO_TypeDef *)Port)->CRH &= ~(0x2UL << (4 * (PIN - 8) + 2));
        }
    }
    ...
    template <
        GPIO_Ports Port,
        unsigned char PIN>
    void GPIO<Port, PIN>::InitInputPullDown()
    {
        if constexpr (Port == GPIOA_BASE)
        {
            RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;
    ...
    template <
        GPIO_Ports Port,
        unsigned char PIN>
    void GPIO<Port, PIN>::InitInputPullUp()
    {
        if constexpr (Port == GPIOA_BASE)
        {
            RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;
